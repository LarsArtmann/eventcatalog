...
export const getLogsForEvent = ({
  ...
  allVersions.unshift({
    ...
    pathToDir: path.join(eventsDir, eventName),
  });
  ...
  const versions = allVersions.reduce((diffs, versionData, index) => {
    ...
    if (hasVersionToCompareToo) {
      ...
      const { version: previousVersion, pathToDir: previousVersionPathToDir } =
        previousVersionData;
      ...
      try {
        const { content } = readMarkdownFile(
          path.join(previousVersionPathToDir, "changelog.md"),
        );
        changelog = content;
      } catch (error) {
...
export const getEventByPath = (
  eventDir: string,
  hydrateWithProducersAndConsumers?: boolean,
): Event => {
  ...
  if (hydrateWithProducersAndConsumers) {
    ...
    const consumersAndProducersAsServices = hydrateEventProducersAndConsumers(
      event,
      services,
    );
    return {
...
export const getAllEventsFromPath = (
  eventsDir: string,
  ...
  return folders.map((folder) =>
    getEventByPath(path.join(eventsDir, folder), hydrateEvents),
  );
};

export const getAllEvents = ({
  hydrateEvents,
}: { hydrateEvents?: boolean } = {}): Event[] => {
  const allEventsFromDomainFolders = getAllEventsFromDomains(hydrateEvents);
  const eventsWithoutDomains = getAllEventsFromPath(
    path.join(process.env.PROJECT_DIR, "events"),
    hydrateEvents,
  );
...
export const getAllOwners = async (): Promise<string[]> => {
  ...
  const allOwnersInDomains = allDomains.reduce(
    (owners, file) => owners.concat(file.domain.owners),
    [],
  );
  const allOwnersInEvents = allEvents.reduce(
    (owners, event) => owners.concat(event.owners),
    [],
  );
  const allOwnersInServices = allServices.reduce(
    (owners, service) => owners.concat(service.owners),
    [],
  );
  const allOwnersDocumented = [].concat(
    allOwnersInDomains,
    allOwnersInEvents,
    allOwnersInServices,
  );
  ...
  return allEvents.reduce((eventsWithVersionsFlattened: any, event: Event) => {
    ...
    if (event.historicVersions) {
      event.historicVersions.forEach((version) =>
        eventsWithVersionsFlattened.push({
...
export const getEventByName = async ({
  ...
  try {
    const { data, content } = readMarkdownFile(
      path.join(directoryToLoadForEvent, `index.md`),
    );
    const event = parseEventFrontMatterIntoEvent(data);
    const services = getAllServices();
    const consumersAndProducersAsServices = hydrateEventProducersAndConsumers(
    ...
    return {
      event: {
        ...
        examples: getEventExamplesFromDir(
          path.join(directoryToLoadForEvent, `examples`),
        ),
      },
      markdown: {
        content,
...
export const getAllEventsByOwnerId = async (ownerId) => {
  ...
  return events.filter(({ owners = [] }) =>
    owners.some((id) => id === ownerId),
  );
};
...
export const getAllEventsThatHaveRelationshipWithService = (
  ...
  const relationshipsBetweenEvents = events.reduce(
    (data, event) => {
      const serviceSubscribesToEvent = event.consumerNames.some(
        (id) => id === service.name,
      );
      const servicePublishesEvent = event.producerNames.some(
        (id) => id === service.name,
      );

      return {
        publishes: servicePublishesEvent
          ? data.publishes.concat(event)
          : data.publishes,
        subscribes: serviceSubscribesToEvent
          ? data.subscribes.concat(event)
          : data.subscribes,
      };
    },
