...
export const getSchemaFromDir = (pathToSchemaDir: string): Schema => {
  try {
    ...
    const schemaFileName = files.find((fileName) =>
      fileName.includes("schema"),
    );
    if (!schemaFileName) throw new Error("No schema found");

    const schemaFile = fs.readFileSync(
      path.join(pathToSchemaDir, schemaFileName),
      "utf-8",
    );
...
export const getOpenAPISpecFromDir = (pathToOpenAPISpecDir: string): string => {
  try {
    ...
    const openAPIFileName = files.find((fileName) =>
      fileName.includes("openapi"),
    );
    if (!openAPIFileName) throw new Error("No schema found");
    return fs.readFileSync(
      path.join(pathToOpenAPISpecDir, openAPIFileName),
      "utf-8",
    );
  } catch (error) {
    return null;
...
export const getAsyncAPISpecFromDir = (
  pathToAsyncAPISpecDir: string,
): string => {
  try {
    const files = fs.readdirSync(pathToAsyncAPISpecDir);

    // See if any oas are in there, ignoring extension
    const asyncAPIFile = files.find((fileName) =>
      fileName.includes("asyncapi"),
    );
    if (!asyncAPIFile) throw new Error("No async api file found");
    return fs.readFileSync(
      path.join(pathToAsyncAPISpecDir, asyncAPIFile),
      "utf-8",
    );
  } catch (error) {
    return null;
