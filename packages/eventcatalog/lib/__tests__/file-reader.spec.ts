...
describe("file-reader lib", () => {
  ...
  describe("getSchemaFromDir", () => {
    ...
    it("returns null when no schema file can be found in the given directory path", () => {
      const result = getSchemaFromDir(
        path.join(process.env.PROJECT_DIR, "events", "EmailSent"),
      );
  ...
  describe("getOpenAPISpecFromDir", () => {
    it("returns the OPEN API file found in the given directory path", () => {
      ...
      const result = getOpenAPISpecFromDir(
        path.join(process.env.PROJECT_DIR, "services", "Payment Service"),
      );
    ...
    it("returns null when no schema file can be found in the given directory path", () => {
      const result = getSchemaFromDir(
        path.join(process.env.PROJECT_DIR, "services", "Email Service"),
      );
