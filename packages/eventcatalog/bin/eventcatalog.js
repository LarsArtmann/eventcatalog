...
const copyCoreApplicationCodeIntoUsersProjectDir = () => {
  ...
  const exclusions = excludeFilesForCopy.map((file) =>
    path.join(eventCatalogLibDir, file),
  );
...
cli
  ...
  .action(() => {
    ...
    fs.copySync(
      ...
      path.join(eventCatalogLibDir, "public"),
    );
    ...
    if (fs.pathExistsSync(path.join(projectDIR, "eventcatalog.styles.css"))) {
      fs.copyFileSync(
        path.join(projectDIR, "eventcatalog.styles.css"),
        path.join(eventCatalogLibDir, "eventcatalog.styles.css"),
      );
    }

    // Move the schemas into public directory so we can download them from UI
    execSync(
      `cross-env PROJECT_DIR='${projectDIR}' npm run scripts:move-schema-for-download`,
      {
        cwd: eventCatalogLibDir,
        stdio: "inherit",
      },
    ...
    fs.copySync(
      path.join(eventCatalogLibDir, ".next"),
      path.join(projectDIR, ".next"),
    );
    fs.copySync(
      path.join(eventCatalogLibDir, "out"),
