...
const VERSIONED_CATALOG_DIRECTORY = path.join(
  __dirname,
  "./assets/catalog_version_test",
);
...
describe("eventcatalog-utils", () => {
  afterEach(() => {
    try {
      fs.rmdirSync(path.join(CATALOG_DIRECTORY, "events", "My New Event"), {
        recursive: true,
      });
      fs.rmdirSync(
        path.join(
          CATALOG_DIRECTORY,
          "events",
          "My Event That Overrides Content",
        ),
        { recursive: true },
      );
      fs.rmdirSync(
        path.join(CATALOG_DIRECTORY, "events", "My Versioned Event"),
        { recursive: true },
      );
      fs.rmdirSync(path.join(CATALOG_DIRECTORY, "services", "My New Service"), {
        recursive: true,
      });
    } catch (error) {
  ...
  describe("events", () => {
    describe("getEventFromCatalog", () => {
      ...
      it("returns the given event name of a given previous version from the catalog", () => {
        const { content, data, raw } = getEventFromCatalog("OrderCreated", {
          version: "0.0.1",
        });
    ...
    describe("versionEvent", () => {
      it("takes a given event name and versions the event", () => {
        const { versionEvent } = utils({
          catalogDirectory: VERSIONED_CATALOG_DIRECTORY,
        });
        ...
        const { versionedPath } = versionEvent("OrderComplete", {
          removeOnVersion: false,
        });

        expect(fs.existsSync(versionedPath)).toEqual(true);
        expect(fs.existsSync(path.join(versionedPath, "index.md"))).toEqual(
          true,
        );
        expect(fs.existsSync(path.join(versionedPath, "schema.json"))).toEqual(
          true,
        );
...
---`,
        ...
        const { versionedPath } = versionEvent("EventWithCleanVersion", {
          removeOnVersion: true,
        });
    ...
    describe("writeEventToCatalog", () => {
      ...
      it("when writing an event with a schema the default markdown includes the schema component", () => {
        ...
        const { path: eventPath } = writeEventToCatalog(event, {
          schema: {
            extension: "json",
            fileContent: JSON.stringify({ test: true }, null, 4),
          },
        });
      ...
      it("when the same event already exists in the catalog, that markdown content is used in the new event", () => {
        ...
        const newFileContents = fs.readFileSync(
          path.join(eventPath, "index.md"),
          "utf-8",
        );
      ...
      describe("function options", () => {
        it("when given a `schema` the schema is written into the catalog with the event", () => {
          ...
          expect(fs.existsSync(path.join(eventPath, "schema.json"))).toEqual(
            true,
          );
        ...
        it("when given a list of `codeExamples` they are written into the catalog with the event", () => {
          ...
          expect(
            fs.existsSync(path.join(eventPath, "examples", "example1.js")),
          ).toEqual(true);
          expect(
            fs.existsSync(path.join(eventPath, "examples", "example2.js")),
          ).toEqual(true);
        ...
        it("versions the current event in the directory when `versionExistingEvent` is set to true", () => {
          ...
          expect(
            fs.existsSync(path.join(eventPath, "versionsed", "1.0.0")),
          ).toEqual(false);
          ...
          expect(
            fs.existsSync(path.join(updatedEventPath, "versioned", "1.0.0")),
          ).toEqual(true);
        ...
        describe("isLatestVersion", () => {
          it("throws an error when isLatestVersion is false but no version is specified", () => {
            ...
            expect(() =>
              writeEventToCatalog(event, { isLatestVersion: false }),
            ).toThrow("No `version` found for given event");
          });
          ...
          it("should take the given event of a given version and write it into the catalog as a versioned event", () => {
            ...
            expect(fs.existsSync(eventPath)).toEqual(true);
            expect(eventPath).toMatch(
              /events\/OrderCreated\/versioned\/0.0.3$/,
            );
        ...
        describe("frontMatterToCopyToNewVersions", () => {
          it("when automatically versioning an event the frontmatter is copied to the new event with any properties specified", () => {
            ...
            const { path: updatedEventPath } = writeEventToCatalog(
              updatedEvent,
              {
                useMarkdownContentFromExistingEvent: true,
    ...
    describe("existsInCatalog (event)", () => {
      ...
      it("returns true when a given event of a given previous version exists in the catalog", () => {
        const result = existsInCatalog("OrderCreated", {
          type: "event",
          version: "0.0.1",
        });
        expect(result).toEqual(true);
  ...
  describe("services", () => {
    ...
    describe("writeServiceToCatalog", () => {
      it("should take the given service and write it into the catalog", () => {
        ...
        const result = fs.readFileSync(
          path.join(servicePath, "index.md"),
          "utf-8",
        );
      ...
      it("throws an error when the given service does not have a name", () => {
        ...
        ).toThrow("No `name` found for given service");
      });
    });
  });
  ...
  describe("domains", () => {
    ...
    describe("writeDomainToCatalog", () => {
      ...
      it("writes the given domain into the catalog and does not copy of existing content if `useMarkdownContentFromExistingService` is set to false", () => {
        ...
        const { path: domainPath } = writeDomainToCatalog(domain, {
          useMarkdownContentFromExistingDomain: false,
        });
      ...
      it("throws an error when the given domain does not have a name", () => {
        ...
        ).toThrow("No `name` found for given domain");
      });
    });
  });
});
