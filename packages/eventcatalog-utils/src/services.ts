...
import {
  ...
  WriteServiceToCatalogInterface,
  WriteServiceToCatalogInterfaceReponse,
} from "./types";
...
export const getAllServicesFromCatalog =
  ...
  (): any[] => {
    ...
    return folders.map((folder) => {
      ...
      const { raw, ...service }: any = getServiceFromCatalog({
        catalogDirectory,
      })(folder);
      return service;
    });
...
export const getServiceFromCatalog =
  ...
  (seriveName: string) => {
    try {
      ...
      const { parsed: parsedService, raw } = readMarkdownFile(
...
export const writeServiceToCatalog =
  ...
  (
    service: Service,
    options?: WriteServiceToCatalogInterface,
  ): WriteServiceToCatalogInterfaceReponse => {
    ...
    fs.writeFileSync(
      path.join(catalogDirectory, "services", service.name, "index.md"),
      data,
    );
