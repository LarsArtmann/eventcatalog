...
import {
  ...
  WriteDomainToCatalogOptions,
  WriteDomainToCatalogResponse,
} from "./types";
  ...
  (domainName: string) => {
    try {
      ...
      const { parsed: parsedService, raw } = readMarkdownFile(
        path.join(catalogDirectory, "domains", domainName, "index.md"),
      );
      return {
...
export const writeDomainToCatalog =
  ...
  (
    domain: Domain,
    options?: WriteDomainToCatalogOptions,
  ): WriteDomainToCatalogResponse => {
    ...
    fs.writeFileSync(
      path.join(catalogDirectory, "domains", domain.name, "index.md"),
      data,
    );
