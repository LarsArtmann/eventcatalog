...
const getAllEventsFromAsyncDoc = (
  ...
  const allMessages = operations.reduce((data: any, op: OperationInterface) => {
    ...
    const eventsFromMessages = messages.reduce(
      (messagesData: any, message: MessageInterface, i) => {
        ...
        const schema =
          message.extensions().get("x-parser-original-payload")?.value() ||
          message.payload();
        const externalLink = {
...
const parseAsyncAPIFile = async (
  ...
  if (parsed.document === undefined) {
    ...
    parsed.diagnostics
      ...
      .forEach((d) =>
        console.error(
          `${chalk.red(`[${d.code}]`)} ${d.path.join(".")} - ${d.message} - ${pathToFile}:${d.range.start.line}`,
        ),
      );
    console.error();

    throw new Error(
      `Unable to parse the given AsyncAPI document (${pathToFile})`,
    );
  }
...
const writeData = async (
  ...
  const { writeServiceToCatalog, writeEventToCatalog } = utils({
    catalogDirectory: domain
      ? path.join(destDir, "domains", domain.name)
      : destDir,
  });
...
const main = async (_: LoadContext, options: AsyncAPIPluginOptions) => {
  ...
  const data = await Promise.all(
    listOfAsyncAPIFilesToParse.map((specFile) =>
      parseAsyncAPIFile(specFile, options),
    ),
  );
...
if (require.main === module) {
  ...
  if (!process.env.ASYNCAPI_SCHEMAS) {
    throw new Error(
      "Please provide asyncapi schema file (env variable ASYNCAPI_SCHEMAS)",
    );
  }
  ...
  const schemasPaths = schemas.map((f) => {
    ...
    if (schemaPath === undefined) {
      throw new Error(
        `Given schema file not found (looked in ${c.join(", ")})`,
      );
    }
  ...
  main(<LoadContext>{}, {
    pathToSpec: schemasPaths,
    versionEvents: !!process.env.PROJECT_DO_VERSIONS,
  });
}
