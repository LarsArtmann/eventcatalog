      ...
      <div className="max-w-7xl mx-auto py-8 md:py-16 px-4 sm:px-6 lg:py-24 lg:px-8 lg:grid lg:grid-cols-3 lg:gap-x-8">
        <div>
          <p className="mt-2 text-3xl font-extrabold text-gray-200 wide">
            Bring <span className="text-green-500">visibility</span> to your
            Architecture.
          </p>
          ...
          <dl className="space-y-10 sm:space-y-0 sm:grid sm:grid-cols-2 sm:grid-rows-4 sm:grid-flow-col sm:gap-x-6 sm:gap-y-10 lg:gap-x-8">
            {features.map((feature) => (
              <div key={feature.name} className="relative">
                <dt>
                  <feature.icon
                    className="absolute text-gray-200 h-6 w-6"
                    aria-hidden="true"
                  />
                  <p className="ml-9 text-lg leading-6 font-bold text-gray-200">
                    {feature.name}
                  </p>
                </dt>
